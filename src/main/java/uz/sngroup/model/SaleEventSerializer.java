...
public class SaleEventSerializer extends StdSerializer<SaleEvent> {

    private final Calendar cal = Calendar.getInstance();

    public SaleEventSerializer() {
        this(null);
    }

    public SaleEventSerializer(Class<SaleEvent> t) {
        super(t);
    }

    @Override
    public void serialize(SaleEvent event, JsonGenerator jgen, SerializerProvider serializerProvider) throws IOException {

        cal.setTime(event.getDate());

        jgen.writeStartObject();

        jgen.writeStringField("client", event.getCustomer().getName());
        jgen.writeStringField("invoice", event.getInvoice().getNakNo());
